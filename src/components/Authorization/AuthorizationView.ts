...
export default class AuthorizationView extends ElementCreator {
  ...
  drawSignIn(): void {
    ...
    const authIcon = this.createElementSVG(auth, { class: 'auth__icon-wrapper' }, { href: './assets/img/svg/sprite.svg#guest-user', class: 'auth__icon'});
    const authForm = this.createElement('form', auth, { class: 'auth-form' });
      const emailContainer = this.createElement('div', authForm, { class: 'form-item' });
        ...
        const emailField = this.createElement('input', emailContainer, { id: 'email', type: 'email', class: 'form-item__text-input', required: 'required'});
      const passwordContainer = this.createElement('div', authForm, { class: 'form-item' });
        ...
        const passwordField = this.createElement('input', passwordContainer, { id: 'password', type: 'password', class: 'form-item__text-input', minLength: '8', required: 'required'});
  ...
  drawSignUp(): void {
    ...
    const authForm = this.createElement('form', auth, { class: 'auth-form' });
      const genderContainer = this.createElement('div', authForm, { class: 'form-item avatars' });
        const maleInput = this.createElement('input', genderContainer, { id: 'male', name: 'gender', value: 'male', type: 'radio', class: 'form-item__radio-input', required: 'required', checked: 'checked'});
        const maleLabel = this.createElement('label', genderContainer, { for: 'male', class: 'form-item__label' });
        const femaleInput = this.createElement('input', genderContainer, { id: 'female', name: 'gender', value: 'female', type: 'radio', class: 'form-item__radio-input', required: 'required' });
      ...
      const nameContainer = this.createElement('div', authForm, { class: 'form-item' });
        ...
        const nameField = this.createElement('input', nameContainer, { id: 'name', type: 'text', class: 'form-item__text-input', required: 'required'});
      const emailContainer = this.createElement('div', authForm, { class: 'form-item' });
        const emailLabel = this.createElement('label', emailContainer, { for: 'email', class: 'form-item__label' }, 'Электронная почта');
        const emailField = this.createElement('input', emailContainer, { id: 'email', type: 'email', class: 'form-item__text-input', required: 'required'});
      const passwordContainer = this.createElement('div', authForm, { class: 'form-item' });
